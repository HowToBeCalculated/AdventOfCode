...
function is_true_using_add_and_multiple(test_value::Int64, inputs::Vector{Int64}, with_concatenation::Bool = false)::Bool
...
function num_of_true_equations(test_values::Vector{Int64}, input_matrix::Vector{Vector{Int64}}, with_concatenation::Bool = false)::Int64
    summed::Int64 = 0
    for (test_value, inputs) in zip(test_values, input_matrix)
        if is_true_using_add_and_multiple(test_value, inputs, with_concatenation)
            summed += test_value
        end
    end
    return summed
...
if abspath(PROGRAM_FILE) === @__FILE__
    ...
    @time println("Part 1: $(num_of_true_equations(test_values, input_matrix))")
    @time println("Part 2: $(num_of_true_equations(test_values, input_matrix, true))")
end
